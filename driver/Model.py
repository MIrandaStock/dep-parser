...
class ParserModel(nn.Module):
    ...
    def forward(self, words, extwords, tags, masks):
        ...
        # x_lexical: [bz, max_sen_len, word_dim + tag_dim]
        x_lexical = torch.cat((x_embed, x_tag_embed), dim=2)
        # outputs: [max_sen_len, bz, lstm_out_dim] --> [bz, max_sen_len, lstm_out_dim]
        outputs, _ = self.lstm(x_lexical, masks, None)
        outputs = outputs.transpose(1, 0)

        if self.training:
            outputs = drop_sequence_sharedmask(outputs, self.config.dropout_mlp)
        # x_all_dep:  [bz, max_sen_len, MLP_out_dim]
        # x_all_head:  [bz, max_sen_len, MLP_out_dim]

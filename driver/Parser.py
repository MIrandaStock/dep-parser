...
class BiaffineParser(object):
    ...
    def compute_loss(self, true_arcs, true_rels, lengths):
        ...
        index_true_arcs = _model_var(  # 按照最大句子长度pad整个batch的句子, pad 0
            self.model,
            pad_sequence(true_arcs, length=l1, padding=0, dtype=np.int64))
        true_arcs = _model_var(  # 按照最大句子长度pad整个batch的句子, pad -1
            self.model,
        ...
        # F.Cross_entropy(input, target)函数中包含了softmax和log的操作
        arc_loss = F.cross_entropy(
            arc_logits.view(b * l1, l2), true_arcs.view(b * l1),
            ignore_index=-1)

        size = self.rel_logits.size()  # rel_logits: [bz, max_len, max_len, rel_size]
        output_logits = _model_var(
            self.model, torch.zeros(
                size[0], size[1], size[3]))

        for batch_index, (logits, arcs) in enumerate(
                zip(self.rel_logits, index_true_arcs)):
            rel_probs = [] 
            for i in range(l1):
                rel_probs.append(logits[i][int(arcs[i])])  # logits[i][int(arcs[i])]: shape[rel_size]
            rel_probs = torch.stack(rel_probs, dim=0)  # rel_probs: [max_len, rel_size]
            output_logits[batch_index] = torch.squeeze(rel_probs, dim=1)  # [bz, max_len, rel_size]

        b, l1, d = output_logits.size()
        true_rels = _model_var(
            self.model,
            pad_sequence(
                true_rels,
                padding=-1,
    ...
    def parse(self, words, extwords, tags, lengths, masks):
        ...
        for arc_logit, rel_logit, length in zip(
                arc_logits, rel_logits, lengths):
            arc_probs = softmax2d(arc_logit, length, length)
            arc_pred = arc_argmax(arc_probs, length, True)

            rel_probs = rel_logit[np.arange(len(arc_pred)), arc_pred]
            rel_pred = rel_argmax(rel_probs, length, ROOT, True)

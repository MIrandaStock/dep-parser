from data.Vocab import readDepTree
import numpy as np
import torch
...
def data_corpus_weighting(
        data1,
        data2,
        domain_num_corpus_weighting,
        cdt_domain_ration):
...
def batch_data_variable(batch, vocab):
    ...
    for b in range(1, batch_size):  # find the max length of sentences
        if len(batch[b]) > length:
            length = len(batch[b])
    ...
    masks = Variable(torch.LongTensor(batch_size, length).zero_(), requires_grad=False)
    heads = []
    rels = []
    lengths = []
